				   ...
				   bool en)
{
	...
	dev_dbg(&rport->phy->dev, "en: %d vs vbus_en: %d\n",
		en, rport->vbus_enabled);

	if (en && !rport->vbus_enabled) {
		dev_dbg(&rport->phy->dev, "vbus powe regulator is enabled\n");
		ret = regulator_enable(rport->vbus);
		if (ret)
			dev_err(&rport->phy->dev,
				"Failed to enable VBUS supply\n");
	} else if (!en && rport->vbus_enabled) {
		dev_dbg(&rport->phy->dev, "vbus powe regulator is disabled\n");
...
static void rockchip_usb2phy_otg_sm_work(struct work_struct *work)
{
	...
	dev_dbg(&rport->phy->dev, "vbus_attach: %d\n", vbus_attach);
